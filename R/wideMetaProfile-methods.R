...
setMethod("wideMetaProfile",
          ...
          function(sites, forward, reverse=NULL, upstream=100, downstream=100) {
              # Pre-checks
              assertthat::assert_that(assertthat::are_equal(class(forward), class(reverse)),
                                      all(width(sites) == 1),
                                      assertthat::is.count(upstream),
                                      assertthat::is.count(downstream))
              ...
              sites <- promoters(sites, upstream=upstream, downstream=downstream)
              if(any(strand(sites) == "*")) {
                        message('Converting unstranded site(s) to + strand')
              ...
              anti_reverse  <- revElements(x=anti_reverse, i=seq_along(anti_reverse))
